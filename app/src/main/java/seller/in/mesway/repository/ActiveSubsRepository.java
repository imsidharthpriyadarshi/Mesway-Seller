...
public class ActiveSubsRepository {
    ...
    public static ActiveSubsRepository getInstance(Context context){

        return activeSubsRepository = new ActiveSubsRepository(context);

    }

    public ActiveSubsRepository(Context context){
    ...
    private void getTodayMeals(Context context) {
        ...
        Call<TodayMeals> getActiveSubsCall = apiInterface.get_active_subs(Constant.TOKEN_TYPE_VALUE+sharedPreferences.getString(Constant.ACCESS_TOKEN,null), UUID.fromString(sharedPreferences.getString(Constant.MESS_ID,null)));
