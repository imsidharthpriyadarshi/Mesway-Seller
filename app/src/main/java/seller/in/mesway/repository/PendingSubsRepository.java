...
public class PendingSubsRepository {
    ...
    public static PendingSubsRepository getInstance(Context context){

        return pendingSubsRepository = new PendingSubsRepository(context);

    }

    public PendingSubsRepository(Context context){
    ...
    private void getTodayMeals(Context context) {
        ...
        Call<TodayMeals> todayMealsCall = apiInterface.get_pending_subs(Constant.TOKEN_TYPE_VALUE+sharedPreferences.getString(Constant.ACCESS_TOKEN,null), UUID.fromString(sharedPreferences.getString(Constant.MESS_ID,null)));
