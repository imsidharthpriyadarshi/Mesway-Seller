...
public class ActiveExpiredSubsRepository {

    private SharedPreferences sharedPreferences;
    private ApiInterface apiInterface;
    private static ActiveExpiredSubsRepository activeSubsRepository;
    ...
    public static ActiveExpiredSubsRepository getInstance(Context context){

        return activeSubsRepository = new ActiveExpiredSubsRepository(context);

    }

    public ActiveExpiredSubsRepository(Context context){
        todayMealsMutableLiveData=new MutableLiveData<>();
        isLoading= new MutableLiveData<>();
        status_code = new MutableLiveData<>();
        detail = new MutableLiveData<>();
        getTotalEarningBackgroundThread(context);



    }

    public void getTotalEarningBackgroundThread(Context context){
        executor.execute(new Runnable() {
            @Override
            public void run() {
                try {
                    getTotalEarning(context);
                }catch (Exception e){
    ...
    private void getTotalEarning(Context context) {
        ...
        Call<TodayMeals> getActiveSubsCall = apiInterface.get_total_earning(Constant.TOKEN_TYPE_VALUE+sharedPreferences.getString(Constant.ACCESS_TOKEN,null), UUID.fromString(sharedPreferences.getString(Constant.MESS_ID,null)));
        getActiveSubsCall.enqueue(new Callback<TodayMeals>() {
            @Override
